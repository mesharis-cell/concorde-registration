"use client";

import React from "react";
import FormLayout from "../layout/form-layout";
import Container from "../reusable/container";
import { useDynamicStepForm } from "@/hooks/use-dynamic-step-form";
import { DynamicStepField } from "@/components/form/dynamic-step-field";
import { StepNavigation, RegistrationSummary, StepHeader } from "@/components/form";
import Button from "../common/button";
import { WHATSAPP_NUMBER } from "@/constants";

function DynamicStepForm() {
  const {
    currentStep,
    totalSteps,
    currentStepData,
    formValues,
    fieldErrors,
    isSubmitted,
    termsAccepted,
    submitting,
    apiError,
    loading,
    configError,
    handleInputChange,
    handleSwitchChange,
    handleNext,
    handleBack,
    setTermsAccepted,
    resetForm
  } = useDynamicStepForm();

  // Show success summary after submission
  if (isSubmitted) return <RegistrationSummary formValues={formValues} onReset={resetForm} />;

  // Show loading state while fetching form configuration
  if (loading)
    return (
      <Container grain>
        <FormLayout subtitle="LOADING REGISTRATION FORM">
          <div className="flex flex-col items-center justify-center py-20">
            <div className="h-12 w-12 animate-spin rounded-full border-4 border-[#C69143] border-t-transparent"></div>
            <p className="mt-4 text-white">Loading form configuration...</p>
          </div>
        </FormLayout>
      </Container>
    );

  const whatsappMessage = "Hi, I need assistance with registration";
  const whatsappUrl = `https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(whatsappMessage)}`;

  // Show error state if configuration failed to load
  if (configError || !currentStepData)
    return (
      <Container grain>
        <FormLayout subtitle="REGISTRATION UNAVAILABLE">
          <div className="flex flex-col items-center justify-center px-6 py-20 text-white">
            <div>
              <p className="mt-4 text-center font-bold uppercase">
                Unable to load registration form
              </p>
              <p className="mt-2 text-center text-sm text-gray-300 uppercase">
                {configError || "Form configuration is not available"}
              </p>
            </div>

            <Button size="xl" className="mt-6 uppercase" onClick={() => window.location.reload()}>
              Retry
            </Button>
            <div className="mt-4">
              <a href={whatsappUrl} className="flex items-center justify-center gap-1">
                <svg
                  width="12"
                  height="12"
                  viewBox="0 0 14 14"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M11.9031 2.03437C10.5937 0.721875 8.85 0 6.99687 0C3.17187 0 0.059375 3.1125 0.059375 6.9375C0.059375 8.15937 0.378125 9.35312 0.984375 10.4062L0 14L3.67812 13.0344C4.69062 13.5875 5.83125 13.8781 6.99375 13.8781H6.99687C10.8187 13.8781 14 10.7656 14 6.94063C14 5.0875 13.2125 3.34687 11.9031 2.03437ZM6.99687 12.7094C5.95937 12.7094 4.94375 12.4312 4.05937 11.9062L3.85 11.7812L1.66875 12.3531L2.25 10.225L2.1125 10.0063C1.53437 9.0875 1.23125 8.02813 1.23125 6.9375C1.23125 3.75938 3.81875 1.17188 7 1.17188C8.54062 1.17188 9.9875 1.77187 11.075 2.8625C12.1625 3.95312 12.8313 5.4 12.8281 6.94063C12.8281 10.1219 10.175 12.7094 6.99687 12.7094ZM10.1594 8.39062C9.9875 8.30313 9.13438 7.88437 8.975 7.82812C8.81563 7.76875 8.7 7.74062 8.58438 7.91562C8.46875 8.09062 8.1375 8.47813 8.03438 8.59688C7.93438 8.7125 7.83125 8.72812 7.65938 8.64062C6.64063 8.13125 5.97188 7.73125 5.3 6.57812C5.12188 6.27187 5.47812 6.29375 5.80937 5.63125C5.86562 5.51562 5.8375 5.41562 5.79375 5.32812C5.75 5.24063 5.40313 4.3875 5.25938 4.04063C5.11875 3.70313 4.975 3.75 4.86875 3.74375C4.76875 3.7375 4.65312 3.7375 4.5375 3.7375C4.42187 3.7375 4.23437 3.78125 4.075 3.95312C3.91562 4.12812 3.46875 4.54688 3.46875 5.4C3.46875 6.25313 4.09063 7.07813 4.175 7.19375C4.2625 7.30938 5.39687 9.05937 7.1375 9.8125C8.2375 10.2875 8.66875 10.3281 9.21875 10.2469C9.55312 10.1969 10.2437 9.82812 10.3875 9.42188C10.5312 9.01562 10.5313 8.66875 10.4875 8.59688C10.4469 8.51875 10.3313 8.475 10.1594 8.39062Z"
                    fill="white"
                  />
                </svg>
                <span className="pb-0.5 text-xs uppercase">Help Line</span>
              </a>
            </div>
          </div>
        </FormLayout>
      </Container>
    );

  return (
    <Container grain>
      <FormLayout subtitle="REGISTER YOUR INFORMATION HERE">
        {/* Loading Overlay during submission */}
        {submitting && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
            <div className="flex flex-col items-center space-y-4 rounded-lg bg-white p-8">
              <div className="h-8 w-8 animate-spin rounded-full border-4 border-[#C69143] border-t-transparent"></div>
              <p className="font-semibold text-[#C69143]">Submitting your registration...</p>
              <p className="text-center text-sm text-gray-600">Please do not close this window</p>
            </div>
          </div>
        )}

        {/* API Error Display */}
        {apiError && (
          <div className="mb-6 rounded-md border bg-black/40 px-3 py-1.5 text-xs">
            <div className="flex flex-col items-start">
              <h3 className="font-medium text-red-300 uppercase">Registration Error</h3>
              <div className="mt-1 text-red-50 uppercase">{apiError}</div>
            </div>
          </div>
        )}

        <div
          className={`mb-12 w-full space-y-3 sm:mt-4 sm:mb-20 sm:space-y-5 md:px-14 ${submitting ? "pointer-events-none opacity-75" : ""}`}
        >
          <StepHeader
            label={currentStepData?.label}
            subLabel={currentStepData?.subLabel}
            currentStep={currentStep}
            totalSteps={totalSteps}
          />

          <div className="space-y-5 last:space-y-0 sm:space-y-8">
            {currentStepData?.fields.map((field, index) => {
              const fieldValue = formValues[field.name || ""];
              const defaultValue = field.defaultValue;
              // Convert to string or boolean based on field type
              const value =
                fieldValue !== undefined && fieldValue !== null
                  ? fieldValue
                  : field.type === "switch" || field.type === "checkbox"
                    ? defaultValue === true
                      ? true
                      : false
                    : String(defaultValue || "");

              console.log(`🎯 [FIELD RENDER] Field: ${field.name}, Type: ${field.type}`, {
                fieldValue,
                defaultValue,
                computedValue: value,
                allFormValues: formValues
              });

              return (
                <div key={field.name || index} className="dynamic-field-uppercase">
                  <DynamicStepField
                    field={field}
                    index={index}
                    value={value}
                    error={fieldErrors[field.name || ""] || false}
                    onInputChange={handleInputChange}
                    onSwitchChange={handleSwitchChange}
                    formValues={formValues}
                  />
                </div>
              );
            })}
          </div>

          <StepNavigation
            currentStep={currentStep}
            totalSteps={totalSteps}
            formValues={formValues}
            submitting={submitting}
            onNext={handleNext}
            onBack={handleBack}
          />
        </div>
      </FormLayout>
    </Container>
  );
}

export default DynamicStepForm;
